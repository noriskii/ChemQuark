...
begin
    ...
    DB_Integrator.DataModule1.ADOQuery1.SQL.Add('Insert into Professor' +
    ' (Nome, Usuario, Senha, Dica)' +
    ' Values (:A, :B, :C, :D)');

    // Insera-os no banco de dados
    DataModule1.ADOQuery1.Parameters[0].Value := Name.Text;
    DataModule1.ADOQuery1.Parameters[1].Value := Crypt(User.Text);
    DataModule1.ADOQuery1.Parameters[2].Value := Crypt(PassWord.Text);
    DataModule1.ADOQuery1.Parameters[3].Value := Tip.Text;
...
begin
  ...
  begin
    ...
    DataModule1.ADOQuery1.SQL.Add('Select Usuario from Professo' +
    ...
    if Crypt(User.Text) = DataModule1.ADOQuery1.FieldByName('Usuario').AsString then
    begin
