...
uses
  ...
  JogoDistribuicao, jpeg, DB_Integrator, Sound, Vcl.Buttons, Vcl.MPlayer;
...
//==============================================================================
  TPeriodicTable = class(TForm)
    ...
    procedure Btn_propClick(Sender: TObject);
    procedure CursorDblClick(Sender: TObject);
    procedure LitioClick(Sender: TObject);
...
//------------------------------------------------------------------------------
  procedure TPeriodicTable.Btn_propClick(Sender: TObject);
begin
...
procedure TPeriodicTable.LitioClick(Sender: TObject);
begin
...
//------------------------------------------------------------------------------
  procedure TPeriodicTable.CursorDblClick(Sender: TObject);
begin
 // Se Outras Propriedades não for acessível
    if not EnableButton then
    begin
...
procedure TPeriodicTable.CursorKeyboardTimer(Sender: TObject);
  begin
    ...
    Gambz_[1] := Gambz_[1] + 1;
      ...
      if ( GetAsyncKeyState(VK_DOWN) <> 0 ) and ( CursorIndex > 106 ) then
      begin
