...
uses
  ...
  Dialogs, ExtCtrls, jpeg, StdCtrls, pngimage, DB_Integrator, Sound, Vcl.MPlayer;
...
//==============================================================================
  TGeometryGame = class(TForm)
    ...
    Image_forfeitYes: TImage;
    Image_ForfeitNo: TImage;
    RANKtimer: TTimer;
    ...
    procedure SetTime( Minutes : Byte; Seconds : Byte );
    procedure FormShow(Sender: TObject);
    procedure Image_forfeitYesMouseEnter(Sender: TObject);
    procedure Image_ForfeitNoMouseEnter(Sender: TObject);
    procedure Image_forfeitYesClick(Sender: TObject);
    procedure Image_ForfeitNoClick(Sender: TObject);
    procedure RANKtimerTimer(Sender: TObject);
...
//------------------------------------------------------------------------------
// * Definição do tempo máximo de jogo
//------------------------------------------------------------------------------
procedure TGeometryGame.SetTime(Minutes: Byte; Seconds: Byte);
begin
  Minute := Minutes;
  Second := Seconds;
end;
...
//     ID : Elemento
//------------------------------------------------------------------------------
procedure TGeometryGame.SairClick(Sender: TObject);
begin
 // Cria uma instância da classe de seleção de jogos
        ...
        Inicio.Show();
        // Fecha a janela
        FreeAndNil(Geometry.GeometryGame);
end;

procedure TGeometryGame.SairMouseEnter(Sender: TObject);
begin
            Sair.Picture.LoadFromFile('Graphics/Button/sair_on.png');
end;

procedure TGeometryGame.SairMouseLeave(Sender: TObject);
begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          FileName := StrToWide('Saves/' + IntToStr(SavesCount) + '.athos');
...
procedure TGeometryGame.RANKtimerTimer(Sender: TObject);
...
if Minute > 0 then
    ...
    Pontos := Minute * 60 + Second
  else
    // Determina a pontuação de acordo com o tempo ( Segundos )
    Pontos := Second;// 1 - Modo Avaliação
// 2 - Modo Livre
     //DEFINE VARIAVEIS PARA INSERIR NO BD
  name := SendingPoints.SendingPoint.Name.Text+'-Ligação Ionica';
    ...
    ' Values (1, 2,'''+name+''','''+numero_aluno+''','''+serie_aluno+''','+inttostr(pontos)+','+ datetostr(date)+')');
...
lbl_status.Caption:='Você foi mal... Para tirar dúvidas acesse o manual :)';
...
// * Clique ao botão
//------------------------------------------------------------------------------
procedure TGeometryGame.btn_ManualClick(Sender: TObject);
begin
 Inicio := TInicio.Create(Application);
          // Exibe o formulário
          inicio.mode:=5;
...
procedure TGeometryGame.btn_ManualMouseEnter(Sender: TObject);
begin
btn_manual.Picture.LoadFromFile('Graphics/Button/Manual_on.png');
end;

procedure TGeometryGame.btn_ManualMouseLeave(Sender: TObject);
begin
...
begin
  ...
  RenameFile('Saves/' + IntToStr(SavesCount) + '.athos',
...
begin
  ...
  if Minute > 0 then
    ...
    Pontos := Second;
  // Exibe a pontuação
  Pontuacao.Caption := IntToStr(Pontos);
...
lbl_status.Caption:= 'Parabéns! Bom desempenho :)';
lbl_status.Font.Color:= clgreen;
lbl_status.Visible:= true;
end;

procedure TGeometryGame.Image_ForfeitNoClick(Sender: TObject);
begin
 if Cursor = 1 then
      begin
...
procedure TGeometryGame.Image_ForfeitNoMouseEnter(Sender: TObject);
begin
  Cursor := 1;
     Change_Forfeit_Picture();
end;

procedure TGeometryGame.Image_forfeitYesClick(Sender: TObject);
begin
 if Cursor = 2 then
      begin
        // Cria uma instância da classe de seleção de jogos
...
procedure TGeometryGame.Image_forfeitYesMouseEnter(Sender: TObject);
begin
 Cursor := 2;
     Change_Forfeit_Picture();
end;
...
begin
  ...
  for I := 1 to Til do
    DeleteFile('Saves/' + IntToStr(I) + '.athos');
end;

procedure TGeometryGame.JogarNovamenteClick(Sender: TObject);
begin
...
btn_manual.enabled := false;
      ...
      jogarNovamente.Enabled:= false;
       ...
       fadeout.Visible:= false;
          // Torna a janela de pontuação visível
  PontuacaoPicture.Visible := false;
  // Torna a pontuação visível
...
procedure TGeometryGame.JogarNovamenteMouseEnter(Sender: TObject);
begin
                         JogarNovamente.Picture.LoadFromFile('Graphics/Button/jogar_novamente_on.png');
end;

procedure TGeometryGame.JogarNovamenteMouseLeave(Sender: TObject);
begin
...
begin
  ...
  if Forfeit.Visible then
  begin
  Image_forfeitYes.Enabled := true;
  Image_forfeitNo.Enabled := true;
...
procedure TGeometryGame.FormShow(Sender: TObject);
begin
