...
uses DB_Integrator, Sound, Register, Tip, Menu, Master;
...
begin
  Sound.Play_Cancel();
  Self.Close();
  FreeAndNil(RegisterUser);
  FreeAndNil(Master.Admin);
  FreeAndNil(PassWordFaq);
end;
...
begin
  ...
  begin
    ...
    if Crypt(User.Text) = DataModule1.ADOQuery1.FieldByName(
    ...
    begin
      ...
      if Crypt(PassWord.Text) = DataModule1.ADOQuery1.FieldByName(
                                                  'Senha').AsString then
      begin
        // ID do Professor
        DB_Integrator.Professor := DataModule1.ADOQuery1.FieldByName('ID').AsInteger;
        // Permite logar
        CanLogin := True;
        // Sai do loop
        Break;
...
begin
  Sound.Play_Cursor(1);
  PassWordFaq := TPassWordFaq.Create(Self);
